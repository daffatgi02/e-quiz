                ...
                <div class="question">
                    ...
                    @if($answer->question->type === 'multiple_choice')
                        ...
                        <div class="answer-box {{ $isCorrect ? 'correct-answer' : 'wrong-answer' }}">
                            <strong>{{ __('quiz.user_answer') }}:</strong> {{ $userAnswerOption ? $userAnswerOption->option : __('quiz.no_answer') }}
                            {{ $isCorrect ? '✓' : '✗' }}
                        </div>
                    ...
                    <div style="margin-top: 10px; text-align: right;">
                        <strong>{{ __('quiz.points') }}: {{ $answer->points_earned ?? 0 }} / {{ $answer->question->points }}</strong>
                    </div>
                </div>
            @endforeach
        </div>
