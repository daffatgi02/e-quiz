                    ...
                    <form method="POST" action="{{ route('admin.quizzes.store') }}">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('question_type') is-invalid @enderror" id="question_type" name="question_type" required>
                                <option value="multiple_choice" {{ old('question_type') == 'multiple_choice' ? 'selected' : '' }}>{{ __('quiz.multiple_choice') }}</option>
                                <option value="essay" {{ old('question_type') == 'essay' ? 'selected' : '' }}>{{ __('quiz.essay') }}</option>
                                <option value="mixed" {{ old('question_type') == 'mixed' ? 'selected' : '' }}>{{ __('quiz.mixed') }}</option>
                            </select>
                            @error('question_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="single_attempt" name="single_attempt" value="1" {{ old('single_attempt', true) ? 'checked' : '' }}>
